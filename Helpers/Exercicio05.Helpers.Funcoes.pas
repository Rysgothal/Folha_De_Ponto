unit Exercicio05.Helpers.Funcoes;
...
type
  ...
  TFuncoes = class abstract
    ...
    class function SomenteLetras(const pValor: string): string;
...
class function TFuncoes.SomenteLetras(const pValor: string): string;
begin
  for var vChar in pValor do
  begin
    if CharInSet(vChar, ['A'..'Z']) then
    begin
      Result := Result + vChar;
    end;
  end;
